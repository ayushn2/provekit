...
pub type Sha3Sponge = DuplexSponge<Sha3Permutation>;
...
fn hash_pair(l: FieldElement, r: FieldElement) -> FieldElement {
    let mut bytes = Vec::new();
    bytes.extend_from_slice(&l.into_bigint().to_bytes_le());
    bytes.extend_from_slice(&r.into_bigint().to_bytes_le());
...
impl PowStrategy for Sha3PoW {
    fn new(challenge: [u8; 32], bits: f64) -> Self {
        assert!((0.0..60.0).contains(&bits), "bits must be < 60");
        Self { challenge, bits }
    }
    ...
    fn check(&mut self, nonce: u64) -> bool {
        ...
        (leading_zeros as f64) >= self.bits
    }
